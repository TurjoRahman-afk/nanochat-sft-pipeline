...
parser.add_argument("--smoltalk-epochs", type=int, default=1, help="number of epochs of SmolTalk in training mixture (teaches conversational style)")
...
orig_model = model
import platform
_can_compile = device_type == "cuda" and platform.system() != "Windows"
if _can_compile:
    model = torch.compile(model, dynamic=False)
else:
    print0("Skipping torch.compile (not supported on Windows or non-CUDA — requires Triton)")
...
train_tasks = [
    *[SmolTalk(split="train") for _ in range(args.smoltalk_epochs)], # 460K rows of general conversations per epoch
...
print0(f"Training mixture: {len(train_dataset):,} rows (SmolTalk x{args.smoltalk_epochs}, MMLU x{args.mmlu_epochs}, GSM8K x{args.gsm8k_epochs})")
...
def sft_data_generator_bos_bestfit(split, buffer_size=100):
    ...
    def refill_buffer():
        ...
        while len(conv_buffer) < buffer_size:
            ...
            # Skip conversations that are too long to ever fit in a row.
            # Without this, they accumulate in the buffer until it's 100% long conversations,
            # at which point every row becomes pure padding => all targets masked => NaN loss.
    ...
    while True:
        ...
        # NOTE: `it` counts micro-batch pulls; num_iterations is in optimizer steps.
        # Multiply by grad_accum_steps so --num-iterations 500 means 500 optimizer steps.
        it += 1
        ...
        if split == "train":
            ...
            # Only use dataset exhaustion as stop condition when num_iterations is not explicitly set.
            # When num_iterations > 0, the iteration count check above handles stopping.
...
while True:
    ...
    # save checkpoint at the end of the run BEFORE chatcore eval (chatcore eval may crash on Windows
    # due to multiprocessing.Manager() in HumanEval; saving first ensures we never lose the model)
    if last_step:
    ...
    if args.chatcore_every > 0 and (last_step or (step > 0 and step % args.chatcore_every == 0)):
        ...
        # HumanEval requires multiprocessing.Manager() which crashes on Windows when the script
        # is not protected by `if __name__ == '__main__':`. Skip it on Windows.
        if sys.platform == 'win32':
            all_tasks = [t for t in all_tasks if t != 'HumanEval']
            print0("(Skipping HumanEval on Windows — requires multiprocessing guard)")
        ...
        def centered_mean(tasks):
            return sum((task_results[t] - baseline_accuracies[t]) / (1.0 - baseline_accuracies[t]) for t in tasks if t in task_results) / max(len([t for t in tasks if t in task_results]), 1)
        chatcore = centered_mean(all_tasks)
        chatcore_cat = centered_mean(list(categorical_tasks))
    ...
    for micro_step in range(grad_accum_steps):
        ...
        if torch.isnan(train_loss) or torch.isinf(train_loss):
            # NaN/inf loss — don't backprop (NaN gradients can't be clipped and corrupt weights).
            # This should not happen after the refill_buffer fix above, but kept as a safety net.
            nan_loss = True
            x, y = next(train_loader) # still advance the data loader
    ...
    # step the optimizer (skip entirely if loss was NaN/inf to prevent corrupting model weights)
    ...
    if not nan_loss:
        ...
        print0(f"Warning: NaN/inf loss at step {step+1}, skipping optimizer step")
    ...
    # logging
    if not nan_loss:  # don't update EMA with NaN values
